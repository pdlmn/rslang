...
const get = genericGet<UserWord[]>((userId) => `${API_URI}/users/${userId}/words`);
...
const getOne = async (userId: string, wordId: string, authToken: string) => {
  ...
  const data = await fetchData<UserWord & { id: string }>(queryString, requestOptions);
  return data;
};
...
const deleteUserWord = async (userId: string, wordId: string, authToken: string) => {
  const queryString = `${API_URI}/users/${userId}/words/${wordId}`;
